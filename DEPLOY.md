...
6. Приложение слушает порт `3000` на сервере. Дальше — HTTPS:
   - **С доменом**: поставьте [Caddy](https://caddyphp.com) (или используйте
     ...
     Let's Encrypt. После этого добавьте в `.env` строку
     `COOKIE_SECURE=true` и пересоздайте контейнер приложения
     (`docker compose -f docker-compose.prod.yml up -d --build app`) —
     иначе вход будет постоянно сбрасываться (браузер не сохраняет
     cookie сессии без HTTPS, см. ниже).
